...
def _main(cfg: DictConfig, output_file):
    ...
    folder_to_save_attn_mat = '/checkpoint/itayitzhak/attn_weigths/' + mod_name + '_' + '_'.join(cfg.task['data'].split('/')[-6:])
    from pathlib import Path
    print(folder_to_save_attn_mat)
    try:
        Path(folder_to_save_attn_mat).mkdir(parents=True, exist_ok=True)
    except Exception as e:
        print(e)
    with open(f'{folder_to_save_attn_mat}/count.txt', 'w+') as f:
        #f.write(folder_to_save_attn_mat+'\n')
        f.write("0\n")
